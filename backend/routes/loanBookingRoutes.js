...
//   // let prefixPartnerLoan = lender === "EV Loan" ? "MANEV1" : "HCFIN1" : "BLFIN1";

//   // let prefixLan = lender === "EV Loan" ? "EV1" : "HC1" : "BL1";
...
//   prefixLan = "EV1";
// } else if (lender === "HC") { // Assuming "HCFIN1" was a typo and should be "HCFIN"
...
//   prefixLan = "GQNonFSF1";
//  }
//  //else if (lender === "Unique ID") {
// //   // prefixPartnerLoan = "UNQID1";
// //   // prefixLan = "ID";
// // }
// else {  
...
//     "SELECT last_sequence FROM loan_sequences WHERE lender_name = ? FOR UPDATE", // ✅ Prevent race conditions

//     [lender]
//   );

//   let newSequence = rows.length > 0 ? rows[0].last_sequence + 1 : 11001;

//   // ✅ Ensure unique sequence by updating it immediately
//   if (rows.length > 0) {
//     // ✅ If lender exists, increment sequence
...
//     );
//   } else {
//     // ✅ If lender does not exist, create a new sequence entry
//     newSequence = 11000;
//     await db
//       .promise()
//       .query(
...
//       if (!disbursementUTR || !disbursementDate || !lan) {
//         console.log(`⚠️ Skipping row due to missing data: ${JSON.stringify(row)}`);
//         continue;
//       }
...
//           `SELECT loan_amount_sanctioned AS loan_amount, interest_percent AS interest_rate, loan_tenure_months AS loan_tenure, subvention_amount,no_of_advance_emis, product, lender 
...
//       const { loan_amount, interest_rate, loan_tenure, subvention_amount,no_of_advance_emis, product, lender } = loanRes[0];
//       console.log(`🔍 Loan details for LAN ${lan}: ${JSON.stringify(loanRes[0])}`);
...
//   await db
//     .promise()
//     .query("UPDATE loan_booking_gq_non_fsf SET status = 'Disbursed' WHERE lan = ?", [lan]);
...
//         processedCount++;
//       } catch (rpsErr) {
//         console.error(`❌ Failed RPS generation for ${lan}, skipping UTR insert`, rpsErr);
//       }
//     }
...
//           message: `Duplicate found for PAN: ${pan} or Aadhaar: ${aadhaar}`,
//         });
//       }
...
//   const query =
//     "SELECT * FROM loan_booking_gq_non_fsf WHERE LAN like 'GQ%'";
...
// //   const lan = req.params.lan;
// //   const query = `SELECT * FROM manual_rps_ev_loan WHERE lan = ? ORDER BY due_date ASC`;
...
//     return res.status(400).json({ message: "LAN is required and cannot be empty" });
//   }

//   // ✅ Choose correct table based on LAN prefix
//   const isGQ = lan.startsWith("GQN");
//   const tableName = isGQ ? "manual_rps_gq_non_fsf" : "manual_rps_ev_loan";

//   console.log(`📦 Using schedule table: ${tableName} for LAN: ${lan}`);

//   const query = `SELECT * FROM ${tableName} WHERE lan = ? ORDER BY due_date ASC`;
...
//     if (results.length === 0) {
//       console.warn(`⚠️ No schedule found for LAN: ${lan}`);
//       return res.status(404).json({ message: "No schedule found for this loan" });
//     }

//     console.log(`✅ Schedule fetched for LAN: ${lan} — ${results.length} entries`);
//     res.json(results);
//   });
// });
