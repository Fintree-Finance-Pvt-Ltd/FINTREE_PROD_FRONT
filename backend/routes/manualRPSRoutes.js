...
// ✅ Check if LAN Exists in `loan_bookings`
const getValidLAN = (lan) => {
    const tablename = lan.startsWith("WCTL") ? "loan_bookings_wctl" : "loan_bookings_ev_loan";
    return new Promise((resolve, reject) => {
...
//         if (skippedEntries.length > 0) {
//             return res.status(207).json({
//                 message: `Upload partially completed. ${dataToInsert.length} rows inserted, ${skippedEntries.length} skipped.`,
//                 skipped: skippedEntries
//             });
//         } else {
//             return res.json({ message: "All rows uploaded successfully." });
